/**
 * Tiếng Việt.
 *
 * Viết cho người không rành công nghệ. Ba nguyên tắc:
 *
 *   **Không để nguyên thuật ngữ nếu tiếng Việt có từ dùng được.** "API key" thì
 *   giữ, vì đó là cái người ta đi copy trên trang của nhà cung cấp và phải nhận ra
 *   được. "Model" cũng giữ, vì mọi nơi khác trong ngành đều gọi thế.
 *
 *   **Nói hậu quả, không nói cơ chế.** "Model miễn phí, đủ cho việc thường ngày"
 *   hữu ích hơn "rate-limited free tier".
 *
 *   **Không hứa quá.** Chỗ nào có đánh đổi thì nói ra, vì người mới là người ít có
 *   khả năng tự phát hiện nhất.
 */
export const vi = {
  /* ── chung ─────────────────────────────────────────────────────── */
  'app.name': 'Synapse',
  'action.next': 'Tiếp tục',
  'action.back': 'Quay lại',
  'action.skip': 'Bỏ qua',
  'action.done': 'Bắt đầu dùng',
  'action.close': 'Đóng',
  'action.save': 'Lưu',
  'action.cancel': 'Huỷ',
  'action.change': 'Đổi',

  /* ── thanh bên ─────────────────────────────────────────────────── */
  'nav.newChat': 'Cuộc trò chuyện mới',
  'nav.search': 'Tìm trong hội thoại',
  'nav.projects': 'Dự án',
  'nav.artifacts': 'Tệp đã tạo',
  'nav.scheduled': 'Chạy theo giờ',
  'nav.workflows': 'Chuỗi việc',
  'nav.workspace': 'Thư mục làm việc',
  'nav.settings': 'Cài đặt',
  'nav.conversations': 'HỘI THOẠI',

  /* ── thanh trên & khung soạn ───────────────────────────────────── */
  'topbar.computers': 'Máy tính',
  'topbar.panel': 'Tiến trình và màn hình',
  'composer.placeholder': 'Hỏi bất cứ điều gì…  (Enter để gửi, Shift+Enter để xuống dòng)',
  // Khi một lượt đang chạy thì ô này làm việc khác, nên nó nói ra: gửi lúc này
  // là xếp hàng đợi — tin nhắn chờ hiện rõ và vào ở ranh giới bước kế tiếp,
  // chứ không mở một cuộc hội thoại thứ hai đè lên cái đang chạy.
  'composer.placeholderRunning': 'Xếp thêm một tin nhắn…',
  // Danh sách việc do `update_plan` vẽ trong một tin nhắn, và số đếm cạnh cùng
  // các bước đó ở thanh tiến trình.
  'chat.plan': 'Kế hoạch',

  /* ── một chuỗi thao tác ─────────────────────────────────────────── */
  'steps.browser': 'Đã dùng trình duyệt',
  'steps.desktop': 'Đã dùng máy tính',
  'steps.count': '{n} bước',
  'step.output': 'Kết quả',
  'step.seconds': '{n} giây',
  'step.browser.open': 'Mở',
  'step.browser.tabs': 'Xem các tab',
  'step.browser.switchTab': 'Chuyển sang tab',
  'step.browser.closeTab': 'Đóng tab',
  'step.browser.look': 'Đọc trang',
  'step.browser.click': 'Bấm',
  'step.browser.type': 'Gõ',
  'step.browser.press': 'Nhấn phím',
  'step.browser.back': 'Quay lại',
  'step.browser.forward': 'Đi tiếp',
  'step.browser.select': 'Chọn',
  'step.browser.hover': 'Rê chuột lên',
  'step.browser.scroll': 'Cuộn',
  'step.browser.wait': 'Chờ',
  'step.browser.close': 'Đóng trình duyệt',
  'step.desktop.windows': 'Xem các cửa sổ',
  'step.desktop.launch': 'Mở',
  'step.desktop.look': 'Nhìn màn hình',
  'step.desktop.focus': 'Chuyển sang',
  'step.desktop.click': 'Bấm',
  'step.desktop.type': 'Gõ',
  'step.desktop.key': 'Nhấn phím',
  'step.desktop.scroll': 'Cuộn',
  'step.desktop.wait': 'Chờ',
  'step.desktop.close': 'Đóng',

  /* ── kết nối một máy tính ────────────────────────────────────────── */
  'worker.copy': 'Sao chép',
  'worker.copied': 'Đã sao chép',
  'setup.warning':
    'Dòng lệnh này trao cho người chạy nó toàn quyền trên máy đó — tệp, shell, và điều khiển màn hình. Chỉ chạy trên máy của chính bạn, và đừng bao giờ dán một dòng do người khác gửi.',
  'setup.expires': 'Dùng được trong {n} phút, và chỉ một lần.',

  /* ── máy tính mà trợ lý thao tác ─────────────────────────────────── */
  'devices.followsYou': 'Trợ lý thao tác trên máy bạn đang mở trang này.',
  'devices.pinned': 'Đang ghim vào một máy, bất kể bạn ngồi ở đâu.',
  'devices.unpin': 'Theo máy tôi đang ngồi',
  'devices.unpinned': 'Sẽ dùng máy bạn đang ngồi.',

  'chat.planCount': '{done}/{total}',

  // Tin nhắn gõ trong lúc trợ lý còn đang chạy, chờ phía trên khung soạn cho
  // đến khi bước hiện tại xong.
  'queue.more': 'Xem thêm',
  'queue.less': 'Thu gọn',
  'queue.now': 'Gửi ngay',
  'queue.nowHint': 'Gửi ngay lập tức, không đợi bước đang chạy kết thúc',
  'queue.remove': 'Xoá tin nhắn đang chờ này',
  'composer.attach': 'Gắn ảnh hoặc tệp',
  'composer.send': 'Gửi',
  'composer.queue': 'Xếp hàng đợi — sẽ vào ở bước kế tiếp',
  'composer.stop': 'Dừng',
  'composer.stopped': 'Đã dừng.',
  'mirror.watching': 'Đang chạy ở tab khác — đang theo dõi…',
  /* Trợ lý đang làm gì, ở dòng ngay trên khung soạn. */
  'status.thinking': 'Đang suy nghĩ…',
  'status.compacting': 'Đang tóm tắt các lượt cũ…',
  'status.tool': 'Đang chạy {name}…',
  // Khác hẳn với suy nghĩ, và chính chỗ khác đó mới đáng nói: nhà cung cấp
  // chưa bắt đầu trả lời. Số giây tăng dần, vì một con số biết nhúc nhích là
  // thứ phân biệt 'đang chờ' với 'không biết chuyện gì'.
  'status.waiting': 'Đang chờ {model} bắt đầu — {n}s',
  'status.waitingFree': 'Đang chờ {model} — model miễn phí phải xếp hàng lúc đông ({n}s). Đổi model nếu lâu quá.',

  /* ── vì sao câu trả lời dừng lại ─────────────────────────────────
   *
   * Chỉ hiện khi câu trả lời *chưa* trọn vẹn. Ba trong số này trước đây kết
   * thúc y hệt một lượt bình thường: luồng ngắt, vòng quay biến mất, và đoạn
   * cuối đơn giản là không có kết. Nói rõ đã xảy ra chuyện gì chính là khác
   * biệt giữa "trợ lý đã trả lời" và "trợ lý bị cắt ngang".
   */
  'stop.truncated':
    'Bị cắt ngang — câu trả lời chạm giới hạn độ dài đầu ra của model này. Bảo nó viết tiếp, hoặc chọn model có giới hạn đầu ra lớn hơn.',
  'stop.refused':
    'Hệ thống an toàn của nhà cung cấp đã từ chối yêu cầu này nên model không tạo ra gì cả. Hãy diễn đạt lại, hoặc đổi model.',
  'stop.filtered':
    'Bộ lọc nội dung của nhà cung cấp đã chặn câu trả lời này. Phần bạn thấy chỉ là phần lọt qua được.',
  'stop.recitation':
    'Dừng lại vì câu trả lời đang chép lại tài liệu nguồn quá sát. Hỏi lại bằng lời của bạn thì thường sẽ qua.',
  'stop.unknown': 'Nhà cung cấp kết thúc câu trả lời này mà không nói lý do.',

  'approval.title': 'Duyệt các hành động này?',
  'composer.label': 'Tin nhắn',

  /* ── chính khung hội thoại ───────────────────────────────────────
   *
   * Những chuỗi này người dùng gặp ở *mọi* lượt — phần suy luận gập lại, kết
   * quả của một tool, các nút trên một tài liệu — mà trước đây đều là tiếng
   * Anh cứng, trong khi nút đổi ngôn ngữ trong Cài đặt lại nói ngược lại.
   */
  'chat.reasoning': 'Suy luận',
  'chat.noOutput': '(không có kết quả)',
  'chat.noResult': '(không ghi nhận được kết quả)',
  'chat.diagram': 'Sơ đồ',
  'chat.open': 'Mở',
  'chat.download': 'Tải về',
  'chat.openNamed': 'Mở {name}',
  'chat.copy': 'Sao chép',
  'chat.edit': 'Sửa',
  'chat.summaryFold': 'Đọc bản tóm tắt',
  'chat.compacted': 'Đã tóm tắt {n} tin nhắn cũ để lấy thêm chỗ',
  'chat.compactedOne': 'Đã tóm tắt 1 tin nhắn cũ để lấy thêm chỗ',

  /* ── những gì hiện ra trong lúc một lượt đang chạy ───────────── */
  'status.reconnecting': 'Đang kết nối lại…',
  'status.restarting': 'Đang viết lại câu trả lời đó…',
  'status.paused': 'Đã tạm dừng sau nhiều lần nối lại. Gửi một tin nhắn để chạy tiếp.',
  'status.streamFailed': 'Luồng dữ liệu bị lỗi.',
  'status.queued': 'Đã gửi — trợ lý sẽ đọc ở bước kế tiếp.',
  'status.pickedUp': 'Đã tiếp nhận: "{text}"',
  'status.folded': 'Đã gộp {n} tin nhắn cũ thành một bản tóm tắt để lấy thêm chỗ.',
  'usage.tokens': '{n} token',
  'usage.thisTurn': 'lượt này',
  'usage.cached': '{n}% từ cache',
  /* ── chip chế độ, ngay cạnh nút Gửi ──────────────────────────────
   *
   * Năm chế độ và câu giải thích cho từng cái. Nó nằm ngay cạnh nút gửi — thứ
   * được nhìn nhiều nhất trong app — mà trước đây luôn là tiếng Anh.
   */
  'policy.guarded.label': 'Có kiểm soát',
  'policy.ask.label': 'Hỏi trước',
  'policy.auto.label': 'Tự chạy',
  'policy.plan.label': 'Lập kế hoạch',
  'policy.readonly.label': 'Chỉ đọc',
  'policy.guarded.hint':
    'Đọc, sửa file trong workspace, điều khiển trình duyệt và các lệnh thường ngày đều chạy thẳng. ' +
    'Xoá, ghi ra ngoài workspace, đụng vào đường dẫn hệ thống Windows và đóng cửa sổ chưa lưu thì dừng lại hỏi. ' +
    'Phép kiểm tra đó là một danh sách các mẫu nguy hiểm đã biết, không phải sandbox — thứ phá hoại mà nó chưa biết vẫn sẽ chạy.',
  'policy.auto.hint': 'Không chặn gì cả, kể cả hành động phá huỷ. Nhanh nhất, và cũng là cái có thể làm mất việc.',
  'policy.ask.hint': 'Mọi thay đổi đều chờ bạn duyệt. An toàn nhất, và cũng làm phiền nhất — bạn sẽ bị hỏi rất nhiều.',
  'policy.plan.hint': 'Tìm hiểu và đọc, rồi trả về một kế hoạch thay vì tự làm. Không có gì trên máy bạn bị thay đổi.',
  'policy.readonly.hint': 'Trợ lý được xem, nhưng các tool có thể thay đổi bất cứ thứ gì thì không bao giờ được đưa cho nó.',

  /* Model được yêu cầu nghĩ kỹ tới mức nào, năm nấc từ rẻ tới cẩn thận. */
  'effort.low': 'Thấp',
  'effort.medium': 'Vừa',
  'effort.high': 'Cao',
  'effort.xhigh': 'Rất cao',
  'effort.max': 'Tối đa',
  /* ── các kệ: Dự án, Tệp đã tạo, Chạy theo giờ ────────────────────
   *
   * Bốn màn hình gọi t() đúng 0 lần. Hai từ điển vẫn khớp nhau suốt thời gian
   * đó — chính vì vậy test:i18n không hề phát hiện ra.
   */
  'pages.projects.title': 'Dự án',
  'pages.projects.new': 'Dự án mới',
  'pages.sortBy': 'Sắp xếp theo',
  'pages.filterBy': 'Lọc theo',
  'pages.order.updated': 'Sửa gần nhất',
  'pages.order.created': 'Ngày tạo',
  'pages.order.name': 'Tên',
  'pages.order.archived': 'Đã lưu trữ',
  'pages.projects.noneMatch': 'Không có dự án nào khớp.',
  'pages.projects.none': 'Chưa có dự án nào.',
  'pages.projects.noneHint':
    'Một dự án gom hướng dẫn và tài liệu của nó vào một chỗ, và mọi hội thoại mở trong đó đều trả lời dựa trên các tài liệu ấy.',
  'pages.projects.archivedNoneMatch': 'Không có dự án lưu trữ nào khớp.',
  'pages.projects.archivedNone': 'Chưa lưu trữ gì.',
  'pages.pinned': 'Đã ghim',
  'pages.optionsFor': 'Tuỳ chọn cho {name}',

  'pages.artifacts.title': 'Tệp đã tạo',
  'pages.artifacts.new': 'Tệp mới',
  'pages.artifacts.none': 'Chưa có tệp nào.',
  'pages.artifacts.noneMatch': 'Không có gì khớp.',
  'pages.artifacts.newHint': 'Cứ nói bạn muốn tạo gì — một báo cáo, một bảng tính, một trang nhỏ.',
  'pages.kind.all': 'Tất cả',
  'pages.kind.page': 'Trang',
  'pages.kind.code': 'Mã nguồn',
  'pages.kind.document': 'Văn bản',
  'pages.kind.sheet': 'Bảng tính',
  'pages.kind.deck': 'Slide',

  'pages.tasks.title': 'Việc chạy theo giờ',
  'pages.tasks.new': 'Việc mới',
  'pages.tasks.none': 'Chưa đặt việc nào.',
  'pages.order.next': 'Lần chạy tới',
  'pages.tasks.describe': 'Mô tả cho trợ lý',
  'pages.tasks.describeHint': 'Nói nó chạy gì và khi nào — "8 giờ sáng mỗi ngày trong tuần, tìm…"',
  'pages.tasks.manual': 'Tự thiết lập',
  'pages.tasks.scheduled': 'Đã đặt lịch.',
  'pages.tasks.openResult': 'Mở kết quả',
  'pages.tasks.pause': 'Tạm dừng',
  'pages.tasks.resume': 'Chạy lại',
  'pages.tasks.remove': 'Xoá',
  'pages.tasks.removeConfirm': 'Xoá nhé?',
  'pages.tasks.every': 'mỗi {cron}',
  'pages.tasks.once': 'một lần',
  'pages.tasks.next': 'lần tới {when}',
  'pages.tasks.paused': 'đang tạm dừng',
  'pages.tasks.last': 'lần trước {status}',

  /* Bốn gợi ý mở đầu hiện ra khi kệ Chạy theo giờ còn trống. */
  'pages.idea.briefing.name': 'Tóm tắt buổi sáng',
  'pages.idea.briefing.what': 'Đêm qua có gì thay đổi trong những thứ bạn theo dõi, đã tìm và tóm tắt sẵn.',
  'pages.idea.briefing.when': 'Các ngày trong tuần lúc 08:00',
  'pages.idea.watch.name': 'Theo dõi một chủ đề',
  'pages.idea.watch.what': 'Kiểm tra tin tức về một chủ đề, và chỉ lên tiếng khi thật sự có gì mới.',
  'pages.idea.watch.when': 'Hằng ngày lúc 09:00',
  'pages.idea.report.name': 'Báo cáo tuần',
  'pages.idea.report.what': 'Một file Word tóm tắt cả tuần, tạo sẵn và để lại trong hội thoại.',
  'pages.idea.report.when': 'Thứ sáu lúc 16:00',
  'pages.idea.tests.name': 'Kiểm tra thư mục làm việc',
  'pages.idea.tests.what': 'Chạy test trên máy bạn và báo lại cái nào hỏng.',
  'pages.idea.tests.when': 'Các ngày trong tuần lúc 09:00',
  /* Thời gian tương đối, và hai con số trên thẻ dự án. Tiếng Việt không biến
     đổi danh từ theo số nhiều, nên đây là cả cụm chứ không phải gốc từ cộng
     's' — cũng chính vì thế mà `plural()` không dịch thẳng được. */
  'when.justNow': 'vừa xong',
  'when.minutes': '{n} phút trước',
  'when.hours': '{n} giờ trước',
  'when.yesterday': 'hôm qua',
  'when.days': '{n} ngày trước',
  'count.sources': '{n} nguồn',
  'count.sourcesOne': '1 nguồn',
  'count.conversations': '{n} hội thoại',
  'count.conversationsOne': '1 hội thoại',
  'count.messages': '{n} tin nhắn',
  'count.messagesOne': '1 tin nhắn',
  'count.pages': '{n} trang',
  'count.pagesOne': '1 trang',
  'pages.tasks.localOnly':
    'Việc theo giờ chỉ chạy khi app này đang mở. Khi triển khai lên server thì chúng chạy mà không cần app.',
  /* ── chuỗi việc ──────────────────────────────────────────────── */
  'wf.title': 'Chuỗi việc',
  'wf.new': 'Chuỗi việc mới',
  'wf.order.recent': 'Thêm gần đây',
  'wf.describe': 'Mô tả cho trợ lý',
  'wf.describeHint':
    'Nói các bước theo thứ tự — "mỗi thứ hai: lấy số liệu, vẽ biểu đồ, gửi email cho nhóm".',
  'wf.manual': 'Tự thiết lập',
  'wf.none': 'Chưa có chuỗi việc nào.',
  'wf.noneHint':
    'Dùng khi một công việc có nhiều chặng bắt buộc theo thứ tự — và khi lỡ lặp lại một chặng thì sẽ thành chuyện. Nếu chỉ có một chỉ dẫn thì đó là việc chạy theo giờ, không phải chuỗi việc.',
  'wf.pause': 'Tạm dừng',
  'wf.resume': 'Chạy lại',
  'wf.remove': 'Xoá',
  'wf.removeConfirm': 'Xoá nhé?',
  'wf.runNow': 'Chạy ngay',
  'wf.running': 'Đang chạy…',
  'wf.finished': 'Xong.',
  'wf.startedBackground': 'Đã bắt đầu — nó sẽ chạy tiếp ở nền.',
  'wf.openResult': 'Mở kết quả',
  'wf.edit': 'Sửa',
  'wf.paused': 'đang tạm dừng',
  'wf.lastRun': 'lần chạy trước {status}',
  'wf.step.pending': 'đang chờ',
  'wf.step.running': 'đang chạy',
  'wf.step.done': 'xong',
  'wf.step.failed': 'thất bại',
  'wf.step.unknown': 'bị gián đoạn — không chạy lại',
  'wf.run.running': 'đang chạy',
  'wf.run.done': 'đã xong',
  'wf.run.failed': 'dừng vì lỗi',
  'wf.run.needs_attention': 'đang chờ bạn',
  'wf.run.cancelled': 'đã huỷ',
  'wf.run.unknown': 'không rõ',
  'wf.lede': 'Nhiều bước, theo thứ tự, chạy mà không cần ai trông. Mỗi bước thấy được kết quả của bước trước, và một lượt chạy bị gián đoạn sẽ tiếp tục từ chỗ dừng chứ không bắt đầu lại.',
  'wf.order.name': 'Tên',
  'wf.everyCron': 'mỗi {cron}',
  'wf.onDemand': 'chạy khi bạn bấm',
  'wf.stopped': 'Đã dừng: {status}.',
  'wf.neverRun': 'chưa chạy lần nào',
  'wf.formEdit': 'Sửa chuỗi việc',
  'wf.formCreate': 'Tạo chuỗi việc',
  'wf.needStep': 'Cần ít nhất một bước — mỗi dòng một chỉ dẫn.',
  'wf.saved': 'Đã lưu.',
  'wf.created': 'Đã tạo.',

  /* ── trình duyệt model ───────────────────────────────────────── */
  'models.allVendors': 'Tất cả nhà cung cấp',
  'models.empty': 'Thư viện đang trống — bấm Làm mới để tải về từ OpenRouter.',
  'models.noMatch': 'Không có gì khớp. Thử ít từ hơn, hoặc dán đầy đủ id của model.',
  'models.builtIn': 'Có sẵn — dùng key nhà cung cấp của bạn',
  'models.onYourKey': '{provider} — trên key của bạn',
  'models.refreshing': 'Đang làm mới…',
  'models.refreshNow': 'Làm mới ngay',
  'models.automatic': 'Tự động',
  'models.autoName': 'Auto — bộ định tuyến miễn phí của OpenRouter',
  'models.autoMeta':
    'openrouter/free · OpenRouter tự chọn một model miễn phí cho từng tin nhắn, kể cả model đọc được ảnh khi bạn đính kèm ảnh. Cần key OpenRouter.',

  /* ── danh sách tệp và trình sửa trong thư mục làm việc ───────── */
  'ws.deleteConfirm': 'Xoá nhé?',
  'ws.renamePrompt': 'Đổi tên hoặc di chuyển — sửa đường dẫn:',
  'ws.moved': 'Đã chuyển.',
  'ws.deleted': 'Đã xoá.',
  'ws.leaveUnsaved': 'Thoát mà không lưu?',
  'ws.saving': 'Đang lưu…',
  'ws.saved': 'Đã lưu.',
  'ws.newFilePrompt': 'Tệp mới — đặt tên, kèm đường dẫn nếu muốn để trong thư mục:',
  /* ── trang dự án ─────────────────────────────────────────────── */
  'proj.pin': 'Ghim',
  'proj.unpin': 'Bỏ ghim',
  'proj.pinned': 'Đã ghim lên đầu.',
  'proj.unpinned': 'Đã bỏ ghim.',
  'proj.pinAria': 'Ghim dự án',
  'proj.unpinAria': 'Bỏ ghim dự án',
  'proj.editDetails': 'Sửa thông tin',
  'proj.archive': 'Lưu trữ',
  'proj.restore': 'Khôi phục',
  'proj.archived': 'Đã lưu trữ. Nó nằm ở kệ lưu trữ, mọi thứ bên trong vẫn còn nguyên.',
  'proj.restored': 'Đã đưa trở lại kệ.',
  'proj.delete': 'Xoá',
  'proj.deleted': 'Đã xoá.',
  'proj.answersFrom': 'Trả lời từ {sources}',
  'proj.answersFirstFrom': 'Ưu tiên trả lời từ {sources}',
  'proj.noSources': 'Chưa có nguồn nào — trả lời như một hội thoại bình thường',
  'proj.untitled': 'Chưa đặt tên',
  'proj.editInstructions': 'Sửa hướng dẫn',
  'proj.instructionsSaved': 'Đã lưu hướng dẫn.',
  'proj.uploadFromDevice': 'Tải lên từ máy',
  'proj.addTextContent': 'Thêm nội dung văn bản',
  'proj.nothingToAdd': 'Không có gì để thêm.',
  'proj.pastedText': 'Văn bản đã dán',
  'proj.added': 'Đã thêm.',
  'proj.addedSources': 'Đã thêm {sources}.',
  'proj.fallbackName': 'Dự án',

  /* ── trình xem tệp đã tạo ────────────────────────────────────── */
  'viewer.noStorage': 'Khung này không có nơi lưu.',
  'viewer.markdownNote': 'Markdown cho văn bản, còn mã nguồn thì hiện đúng tệp gốc.',
  'viewer.versionNote': 'Một phiên bản cũ, hiện đúng như lúc đó. Khôi phục lại thì mới sửa được.',
  'viewer.unreadable': 'Không đọc được tệp này.',
  'viewer.tab.preview': 'Xem trước',
  'viewer.tab.code': 'Mã nguồn',
  'viewer.tab.source': 'Nguồn',
  'viewer.kind.sheets': 'Trang tính',
  'viewer.kind.slides': 'Slide',
  'viewer.kind.pages': 'Trang',
  'viewer.kind.document': 'Tài liệu',
  'viewer.open': 'Mở',
  'viewer.openIn': 'Mở bằng {app}',
  'viewer.openInDefault': 'Mở bằng ứng dụng mặc định',
  'viewer.copy': 'Sao chép',
  'viewer.download': 'Tải về',
  'viewer.showInFolder': 'Mở thư mục chứa',
  'viewer.copyPicture': 'Sao chép ảnh',
  'viewer.copyFormatted': 'Sao chép kèm định dạng',
  'viewer.print': 'In — hoặc lưu thành PDF',
  'viewer.noComputer': 'Chưa kết nối máy tính nào',
  'viewer.noComputerHint':
    'Kết nối một máy tính để mở tệp bằng Word, Excel hoặc mở thư mục. Bấm vào chip ở thanh trên.',
  'viewer.nothingToCopy': 'Tệp này không có gì để sao chép.',
  'viewer.copiedRich': 'Đã sao chép — dán vào Word là giữ nguyên định dạng.',
  'viewer.copied': 'Đã sao chép.',
  'viewer.copyRefused': 'Trình duyệt không cho sao chép. Hãy nhấn Ctrl/⌘+C.',
  'viewer.pictureCopied': 'Đã sao chép ảnh.',
  'viewer.pictureCopyRefused': 'Trình duyệt không cho sao chép ảnh. Hãy tải nó về.',
  'viewer.saving': 'Đang lưu…',
  'viewer.backToPanel': 'Về lại bảng bên',
  'viewer.fullSize': 'Toàn màn hình',
  'viewer.opening': 'Đang mở…',
  'viewer.couldNotOpen': 'Không mở được tệp đó.',

  /* ── bảng màn hình trực tiếp ─────────────────────────────────── */
  'screen.title': 'Màn hình',
  'screen.wholeMachine': 'Toàn bộ màn hình của máy đang chạy worker.',
  'screen.sandboxClosed': 'Đã đóng trình duyệt sandbox.',
  'screen.driveOn': 'Ngừng điều khiển trang (hoặc bấm Escape)',
  'screen.driveOff': 'Tự điều khiển — bấm và gõ thẳng vào trang',
  'screen.close': 'Đóng trình duyệt hộp cát',
  'screen.expand': 'Toàn màn hình',
  'screen.hide': 'Ẩn',
  'ws.renameAria': 'Đổi tên {name}',
  'ws.deleteAria': 'Xoá {name}',
  'ws.renameTitle': 'Đổi tên hoặc di chuyển',
  'proj.memoryScope': 'Ghi nhớ được lưu theo tài khoản, không theo từng dự án.',
  'proj.accountWide': 'toàn tài khoản',
  'proj.addContext': 'Thêm ngữ cảnh',
  'proj.removeAria': 'Xoá {name}',
  'proj.needName': 'Dự án cần có tên.',
  'proj.deleteConfirm':
    'Xoá “{name}”?\n\nCác nguồn của nó sẽ mất theo. Những hội thoại đã mở trong dự án thì vẫn giữ — chúng quay về danh sách thường.',
  'count.chars': '{n} ký tự',
  'count.charsK': '{n}K ký tự',
  'count.charsM': '{n}M ký tự',
  'screen.sandboxNote': 'Cửa sổ trình duyệt riêng của trợ lý — tách biệt với trình duyệt bạn đang dùng.',





  'empty.title': 'Hôm nay bạn muốn làm gì?',
  'empty.body':
    'Chọn model rồi hỏi bất cứ điều gì. Khi máy tính của bạn đã kết nối, trợ lý có thể đọc tệp, sửa code và chạy lệnh ngay trên máy đó.',

  /* ── model ─────────────────────────────────────────────────────── */
  'model.free.tooltip':
    '{model} — model miễn phí. Đủ cho hỏi đáp thường ngày; bị giới hạn số lần gọi và yếu hơn ở việc nhiều bước. Bấm để đổi.',
  'model.switched': 'Đang dùng {model}.',

  /* ── onboarding ────────────────────────────────────────────────── */
  'onb.title': 'Bắt đầu với Synapse',
  'onb.step': 'Bước {n}/{total}',
  'onb.reopen': 'Xem lại hướng dẫn ban đầu',

  'onb.1.title': 'Synapse làm được gì cho bạn',
  'onb.1.body': 'Đây không phải chatbot chỉ biết trả lời. Nó làm việc thật trên máy tính của bạn.',
  'onb.1.a': 'Đọc và sửa tệp, chạy lệnh, mở web — trên máy tính của bạn, bạn xem trực tiếp.',
  'onb.1.b': 'Tự làm báo giá, báo cáo, bảng tính, slide và gửi lại cho bạn tải về.',
  'onb.1.c': 'Điều khiển được từ điện thoại, máy tính bảng hay laptop khác.',
  'onb.1.note':
    'Bạn luôn thấy nó đang làm gì, và những việc nguy hiểm thì nó dừng lại hỏi bạn trước.',

  'onb.2.title': 'Dán một API key vào',
  'onb.2.body':
    'API key là mật khẩu để trợ lý gọi được model AI. Bạn tự lấy trên trang của nhà cung cấp, dán vào đây một lần là xong.',
  'onb.2.recommend':
    'Gợi ý cho người mới: dùng OpenRouter. Một key duy nhất là với tới được gần như mọi model, và có nhiều model miễn phí.',
  'onb.2.open': 'Mở phần dán key',
  'onb.2.done': 'Xong — tài khoản của bạn đã có key.',
  'onb.2.pending': 'Chưa có key nào. Chưa dán thì trợ lý chưa trả lời được.',
  'onb.2.safety': 'Key được mã hoá trên máy chủ của bạn và không bao giờ gửi về trình duyệt.',

  'onb.3.title': 'Model bạn đang dùng',
  'onb.3.free':
    'Bạn đang dùng model MIỄN PHÍ. Đủ tốt cho hỏi đáp, viết lách và việc thường ngày.',
  'onb.3.freeWarn':
    'Cần nói thật: model miễn phí bị giới hạn số lần gọi và làm việc nhiều bước kém hơn. Nếu một việc dài bị dừng giữa đường, đổi sang model trả phí là hết.',
  'onb.3.paid': 'Bạn đang dùng model trả phí — tính vào key của chính bạn.',
  'onb.3.change': 'Đổi model',
  'onb.3.note':
    'Chỉ có một model dùng cho toàn bộ ứng dụng, nên đổi ở đâu cũng là đổi ở mọi nơi.',

  'onb.4.title': 'Thử câu đầu tiên',
  'onb.4.body': 'Bấm một câu bên dưới là nó chạy luôn. Bạn xem nó làm từng bước.',
  'onb.4.try1': 'Máy tính của tôi đang chậm, kiểm tra xem vì sao',
  'onb.4.try2': 'Tìm giúp tôi 5 tin công nghệ đáng chú ý tuần này',
  'onb.4.try3': 'Làm cho tôi một file Excel theo dõi chi phí hàng tháng',
  'onb.4.note': 'Cứ nói bằng tiếng Việt bình thường. Không cần câu lệnh đặc biệt.',

  'onb.5.title': 'Mức độ tự do của trợ lý',
  'onb.5.body': 'Nút cạnh chỗ gửi tin quyết định việc gì nó tự làm, việc gì phải hỏi bạn.',
  'onb.5.guarded':
    'Có kiểm soát (nên dùng) — việc thường ngày thì tự làm; xoá tệp hay sửa ngoài thư mục làm việc thì dừng lại hỏi bạn.',
  'onb.5.auto': 'Tự chạy hết — nhanh nhất, và là mức có thể làm mất dữ liệu.',
  'onb.5.ask': 'Hỏi mọi thứ — an toàn nhất, nhưng bị hỏi rất nhiều.',
  'onb.5.honest':
    'Nói thẳng: phần kiểm tra "việc nguy hiểm" dựa trên danh sách các mẫu lệnh đã biết, không phải một cái lồng kín. Việc phá hoại theo cách lạ vẫn có thể chạy. Cần chắc chắn thì chọn "Hỏi mọi thứ".',
  'onb.5.finish': 'Xong. Bắt đầu làm việc.',

  /* ── cài đặt ───────────────────────────────────────────────────── */
  'settings.title': 'Cài đặt',
  'settings.tab.providers': 'Nhà cung cấp',
  'settings.tab.languages': 'Ngôn ngữ',
  'settings.tab.behaviour': 'Cách hoạt động',
  'settings.tab.skills': 'Kỹ năng',
  'settings.tab.tasks': 'Chạy theo giờ',
  'settings.tab.connectors': 'Kết nối',
  'settings.tab.worker': 'Máy tính',
  'settings.tab.account': 'Tài khoản',
  'settings.tab.people': 'Người dùng',
  'settings.language.label': 'Ngôn ngữ',
  'settings.language.hint': 'Áp dụng ngay, và theo tài khoản của bạn trên mọi thiết bị.',
  'settings.help.label': 'Hướng dẫn',
  'settings.help.hint': 'Xem lại hướng dẫn 5 bước dành cho người mới bắt đầu.',
/* ── MCP ───────────────────────────────────────────────────────── */
  'settings.tab.mcp': 'Máy chủ MCP',
  'mcp.lede':
    'MCP server bổ sung công cụ từ bên ngoài ứng dụng — Figma, Jira, Sentry, cơ sở dữ liệu, và hàng trăm thứ khác. Cắm vào là trợ lý dùng được. Không có gì ở đây do trợ lý tự chọn: bạn tự gõ lệnh, và mọi công cụ từ server đều dừng lại hỏi bạn trước khi chạy.',
  'mcp.name': 'Tên',
  'mcp.transport': 'Kết nối kiểu nào',
  'mcp.transport.stdio': 'Một chương trình trên máy này',
  'mcp.transport.http': 'Một đường dẫn URL',
  'mcp.command': 'Câu lệnh',
  'mcp.command.hint':
    'Toàn bộ câu lệnh, đúng như bạn gõ trong terminal. Việc này chạy một chương trình trên máy bạn, nên chỉ dán thứ bạn tin được.',
  'mcp.url': 'URL của server',
  'mcp.headers': 'Header',
  'mcp.headers.hint': 'Mỗi dòng một cái, dạng Tên: giá trị. Được mã hoá trên máy chủ và không bao giờ gửi lại về trình duyệt.',
  'mcp.add': 'Kết nối',
  'mcp.connected': 'Đã kết nối',
  'mcp.none': 'Chưa có server nào. Trợ lý vẫn dùng đủ công cụ sẵn có của ứng dụng.',
  'mcp.trying': 'Đang thử khởi động server… lần đầu có thể mất một lúc để tải về.',
  'mcp.added': 'Xong — tìm thấy {n} công cụ. Trợ lý dùng được từ lượt sau.',
  'mcp.needName': 'Đặt tên cho server đã.',
  'mcp.needCommand': 'Nhập câu lệnh khởi động server.',
  'mcp.tools': '{n} công cụ',
  'mcp.broken': 'không chạy được',
  'mcp.off': 'đang tắt',
  'mcp.enable': 'Bật',
  'mcp.disable': 'Tắt',
  'mcp.remove': 'Xoá',
  'mcp.suggested': 'Server gợi ý',
  'mcp.suggested.hint': 'Bấm một cái để điền sẵn vào form. Không có gì được cài hay chạy cho tới khi bạn bấm Kết nối.',
  'mcp.needs': 'cần token',
  'mcp.presetReady': 'Đã điền sẵn {name}. Bấm Kết nối để thử.',
  'gate.name': 'Tên của bạn',
  'gate.email': 'Địa chỉ email',
  'gate.password': 'Mật khẩu',
  'gate.totp': 'Mã từ ứng dụng xác thực',
  'gate.resetcode': 'Mã 6 số trong email',
  'gate.newpassword': 'Mật khẩu mới',
  'pages.searchInput': 'Tìm dự án',
  'project.askInput': 'Hỏi về dự án này',
  'composer.attachInput': 'Đính kèm tệp',
  'pair.codeInput': 'Mã ghép nối từ máy tính',
  'models.searchInput': 'Tìm mô hình',
  'models.sortInput': 'Thứ tự sắp xếp',
  'workspace.findInput': 'Tìm trong các tệp này',
  'search.inputLabel': 'Tìm tiêu đề và mọi nội dung',
  'task.whenInput': 'Chạy vào lúc nào',
  'gate.hidePassword': 'Ẩn mật khẩu',
  'gate.showPassword': 'Hiện mật khẩu',
  'gate.sub.reset': 'Nhập mã trong email và chọn mật khẩu mới.',
  'gate.sub.forgot': 'Nhập email của bạn, chúng tôi sẽ gửi mã đặt lại.',
  'gate.sub.first': 'Tạo tài khoản đầu tiên — tài khoản này sẽ là quản trị viên.',
  'gate.sub.signup': 'Tạo tài khoản của bạn.',
  'gate.sub.signin': 'Đăng nhập để tiếp tục.',
  'gate.submit.reset': 'Đặt mật khẩu mới',
  'gate.submit.forgot': 'Gửi mã đặt lại',
  'gate.submit.signup': 'Tạo tài khoản',
  'gate.switch.toSignin': 'Đã có tài khoản? Đăng nhập',
  'gate.switch.toSignup': 'Chưa có tài khoản? Đăng ký',
  'gate.note.firstAccount': 'Chưa ai đăng ký trên bản triển khai này. Hãy tạo tài khoản đầu tiên.',
  'gate.note.resetSent': 'Nếu địa chỉ đó có tài khoản, mã đặt lại đang được gửi. Nhập mã bên dưới.',
  'gate.note.passwordUpdated': 'Đã cập nhật mật khẩu. Đăng nhập bằng mật khẩu mới.',
  'gate.note.noMail': 'Chưa cấu hình dịch vụ email — mã xác nhận nằm trong log của máy chủ.',
  'gate.error.generic': 'Không thực hiện được.',
  'suggest.workspace': 'Cho tôi xem có gì trong workspace và tóm tắt dự án.',
  'suggest.library': 'Tìm trên web xem thư viện này gần đây có gì thay đổi.',
  'suggest.todos': 'Tìm mọi TODO trong mã nguồn và nhóm theo tệp.',
  'suggest.tests': 'Chạy bộ test và giải thích những chỗ hỏng.',
  'chat.openFailed': 'Không mở được cuộc trò chuyện đó.',
  'nav.noConversations': 'Chưa có cuộc trò chuyện nào',
  'chat.deleted': 'Đã xoá cuộc trò chuyện.',
  'chat.answersFrom': 'Trả lời từ',
  'gate.submit.signin': 'Đăng nhập',
  'action.reallyDelete': 'Xoá thật chứ?',
  'action.remove': 'Xoá',
  'chat.newIn': 'Trò chuyện mới — {project}',
  'chat.sourceCount': '{how} {sources} trong “{project}”.',
  'chat.sourcesOne': '{n} nguồn',
  'chat.sources': '{n} nguồn',
  'chat.noSources': '“{project}” chưa có nguồn nào, nên cuộc trò chuyện này trả lời như bình thường.',
  'chat.saveAndAsk': 'Lưu và hỏi lại',
  'chat.summarised': 'Đã tóm tắt {n} tin nhắn trước đó.',
  'connect.step.clone': 'Trên máy đó: clone repo này, rồi <code>npm install</code>.',
  'connect.step.run': 'Chạy {code}',
  'connect.step.codeRemote': 'Nó sẽ hiện một mã ghép nối. Nhập mã ở dưới, hoặc từ nút <strong>Máy tính</strong> trên thanh đầu trang.',
  'connect.step.codeLocal': 'Nó sẽ hiện một mã ghép nối — trừ khi đây chính là máy này, khi đó nó đã kết nối sẵn.',
  'connect.step.codeLocalMore': 'Để thêm một máy <em>khác</em>, hãy chạy <code>npm run connect -- &lt;địa chỉ của ứng dụng này&gt;</code> trên máy đó.',
  'worker.workspaceOnly': 'Công cụ tệp: chỉ trong thư mục làm việc',
  'worker.desktopOn': 'Điều khiển desktop: <strong>bật</strong> — nó điều khiển được ứng dụng thật',
  'keys.saved': 'khoá đã lưu',
  'keys.yours': 'khoá của bạn',
  'keys.shared': 'khoá dùng chung',
  'keys.notSet': 'chưa đặt',
  'keys.triedInOrder': 'thử lần lượt — nếu khoá 1 bị từ chối thì khoá 2 trả lời',
  'keys.addFallback': 'thêm khoá thứ hai và nó thành khoá dự phòng cho khoá này',
  'keys.savedFor': 'Đã lưu khoá {provider}.',
  'keys.removedFor': 'Đã xoá khoá {provider}.',
  'keys.removedFrom': 'Đã xoá khoá khỏi {provider}.',
  'keys.removeAria': 'Xoá khoá {position}',
  'skills.empty': 'Chưa dạy gì cả.',
  'tasks.everyCron': 'mỗi {cron}',
  'tasks.once': 'một lần',
  'tasks.last': 'lần trước: {status}',
  'tasks.next': 'kế tiếp {when}',
  'tasks.paused': 'tạm dừng',
  'tasks.openResult': 'Mở kết quả',
  'tasks.pause': 'Tạm dừng',
  'tasks.resume': 'Tiếp tục',
  'tasks.empty': 'Chưa lên lịch gì.',
  'tasks.scheduled': 'Đã lên lịch. Lần chạy đầu {when}.',
  'connectors.connected': 'đã kết nối',
  'connectors.notConnected': 'chưa kết nối',
  'connectors.connectedAs': 'Đã kết nối với tư cách {account}.',
  'usage.noLimit': 'Không giới hạn — bạn đang dùng khoá API của chính mình.',
  'usage.empty': 'Không dùng gì trong 30 ngày qua.',
  'admin.chatCount': '{n} cuộc trò chuyện',
  'admin.tokensThisMonth': '{n} token tháng này',
  'admin.workerPaired': 'đã ghép máy',
  'admin.unconfirmed': 'chưa xác nhận',
  'admin.suspended': 'đã khoá',
  'admin.limit': 'giới hạn {n}',
  'search.tooShort': 'Hãy nhập ít nhất hai ký tự.',
  'search.noMatch': 'Không có gì khớp với “{query}”.',
  'search.lede': 'Tìm trong các cuộc trò chuyện theo tiêu đề, hoặc theo bất cứ điều gì đã nói trong đó.',
  'chat.answersFirstFrom': 'Trả lời trước hết từ',
  'project.namePrompt': 'Đặt tên cho nó — một môn học, một khách hàng, một bài tập.',
  'clipboard.failed': 'Không truy cập được clipboard — hãy bôi đen đoạn văn bản và bấm Ctrl+C.',
  'chat.stopBeforeEdit': 'Dừng lượt chạy trước đã — sửa tin nhắn sẽ tua lại cuộc trò chuyện.',
  'chat.stillSaving': 'Tin nhắn này còn đang được lưu. Thử lại sau một lát.',
  'composer.empty': 'Tin nhắn không được để trống.',
  'composer.uploading': 'Đang tải lên — chờ một lát.',
  /* Ảnh và tệp đang chờ gửi kèm. */
  'attachment.uploading': 'Đang tải lên…',
  'attachment.remove': 'Bỏ {name}',
  'attachment.tooMany': 'Mỗi lần chỉ gửi được {count} tệp.',
  'attachment.tooBig': '{name} nặng {size}. Giới hạn là {limit}.',
  'vision.blind.one': '{model} không đọc được ảnh, nên ảnh sẽ bị bỏ qua.',
  'vision.blind.many': '{model} không đọc được ảnh, nên các ảnh sẽ bị bỏ qua.',
  'worker.fullDisk': 'Công cụ tệp: toàn bộ ổ đĩa',
  'worker.desktopOff': 'Điều khiển desktop: tắt',
  'worker.noTools': 'Công cụ tệp và shell bị ẩn khỏi trợ lý cho tới khi có máy kết nối.',
  'settings.effort': 'Mức độ suy luận',
  'action.reallyRemove': 'Xoá thật chứ?',
  'account.nameUpdated': 'Đã cập nhật tên.',
  'account.totpOff': 'Đã tắt xác thực hai bước.',
  'twofa.enabled': 'Đang bật',
  'twofa.recoveryLeft': 'còn {count} mã khôi phục',
  'twofa.requiredEveryTime': 'Mỗi lần đăng nhập đều cần ứng dụng xác thực của bạn.',
  'twofa.code': 'Mã',
  'twofa.turnOff': 'Tắt',
  'twofa.notEnabled': 'Chưa bật',
  'twofa.off': 'tắt',
  'twofa.why': 'Yêu cầu mã từ ứng dụng xác thực ở mỗi lần đăng nhập, để mật khẩu bị lộ vẫn chưa đủ để vào.',
  'twofa.setUp': 'Thiết lập xác thực hai lớp',
  'twofa.scanThis': 'Quét mã này bằng ứng dụng xác thực',
  'twofa.cannotScan': 'Không quét được? Nhập khoá này bằng tay:',
  'twofa.onAPhone': 'Trên điện thoại, {link} để mở thẳng ứng dụng xác thực.',
  'twofa.tapHere': 'chạm vào đây',
  'twofa.confirm': 'Xác nhận',
  'twofa.isOn': 'Xác thực hai lớp đã bật',
  'twofa.enabledBadge': 'đã bật',
  'twofa.saveCodes': 'Hãy lưu các mã khôi phục này ngay.',
  'twofa.saveCodesWhy': 'Mỗi mã dùng được một lần, và đó là cách duy nhất vào lại nếu bạn mất điện thoại. Chúng sẽ không hiện lại lần nữa.',
  'twofa.saved': 'Tôi đã lưu rồi',
  'account.passwordUpdatedAll': 'Đã cập nhật mật khẩu. Mọi thiết bị khác đã bị đăng xuất.',
  'account.passwordUpdated': 'Đã cập nhật mật khẩu.',
  'admin.tokenLimit': 'Hạn mức token hằng tháng khi dùng khoá API dùng chung. 0 là không giới hạn.',
  'devices.add': 'Thêm máy tính',
  'devices.thisOne': 'Máy này',
  'devices.yours': 'Máy tính của bạn — thêm máy khác, hoặc đổi máy đang dùng',
  'devices.none': 'Chưa có máy nào kết nối. Bấm để thêm.',
  'devices.waiting': 'Đang chờ được thêm.',
  'devices.willReport': 'Máy sẽ báo nơi nó đang làm việc ngay khi kết nối.',
  'devices.countComputers': '{count} máy',
  'devices.computer': 'Máy tính',
  'devices.waitingAs': 'Đang chờ được thêm với tên “{name}”.',
  'devices.copied': 'Đã chép',
  'devices.copy': 'Chép',
  'devices.pressCtrlC': 'Nhấn Ctrl+C',
  'devices.noneYet': 'Chưa ghép máy nào. Chạy Synapse trên máy bạn muốn dùng rồi nhập mã của nó ở trên.',
  'devices.desktopOn': 'điều khiển desktop: bật',
  'devices.wholeDisk': 'truy cập được toàn bộ ổ đĩa',
  'devices.confined': 'giới hạn trong thư mục làm việc',
  'devices.lastSeen': 'thấy lần cuối {when}',
  'devices.never': 'chưa bao giờ',
  'devices.inUse': 'đang dùng',
  'devices.online': 'trực tuyến',
  'devices.offline': 'ngoại tuyến',
  'devices.workingFolder': 'Thư mục làm việc',
  'devices.workingIn': 'Đang làm việc trong {path}. Xoá trống ô này để trả lại cho thiết lập riêng của máy.',
  'devices.workingInPending': 'Đang làm việc trong {path} — chờ máy nhận thay đổi.',
  'devices.workOnThis': 'Làm việc trên máy này',
  'devices.unpair': 'Bỏ ghép',
  'devices.unpaired': 'Đã bỏ ghép {name}. Không còn kết nối tới máy đó được nữa.',
  'devices.pairing': 'Đang ghép…',
  'devices.added': 'Đã thêm “{name}”. Máy sẽ kết nối trong vài giây.',
  'devices.nowYours': '{name} giờ là của bạn.',
  'time.justNow': 'vừa xong',
  'format.couldNotRead': 'Không đọc được {name}.',
  'api.streamFailed': 'Luồng dữ liệu lỗi, HTTP {status}',
  'screen.newTab': 'thẻ mới',
  'tool.arg.rewriting': 'đang viết lại tài liệu',
  'tool.arg.allNotes': 'toàn bộ ghi chú',
  'project.noInstructions': 'Chưa có gì. Mọi điều viết ở đây sẽ được mang vào mọi cuộc trò chuyện trong dự án này, nên bạn không phải giải thích lại.',
  'time.minShort': '{n} phút trước',
  'time.hoursShort': '{n} giờ trước',
  'time.daysShort': '{n} ngày trước',
  'ws.empty': 'Thư mục này trống.',
  'ws.readingFolder': 'Đang đọc thư mục…',
  'ws.startWorker': 'Hãy chạy worker trên máy bạn muốn làm việc — Cài đặt → Máy tính.',
  'ws.backTo': '← quay lại {where}',
  'ws.noMatch': 'Không có gì trong {where} chứa “{query}”. Đã đọc {scanned} tệp.',
  'ws.hits': '{matches} trong {files}.',
  'ws.matchesOne': '{n} kết quả',
  'ws.matches': '{n} kết quả',
  'ws.filesOne': '{n} tệp',
  'ws.files': '{n} tệp',
  'ws.hitsMore': ' Và còn nữa, vượt quá giới hạn.',
  'viewer.noSheets': 'Bảng tính này không có sheet nào.',
  'viewer.sheetTruncated': 'Sheet này lớn hơn phần xem trước. Hãy tải về để xem phần còn lại.',
  'viewer.noSlides': 'Bộ slide này không có trang nào.',
  'viewer.asSource': 'Đang hiển thị mã nguồn. Tải về để mở trang thật.',
  'viewer.noText': 'Tài liệu này không có chữ để hiển thị — đây là bản quét, hoặc ảnh chụp các trang.',
  'viewer.intact': 'Bản thân tệp vẫn nguyên vẹn — hãy tải về và mở bằng ứng dụng gốc của nó.',
  'viewer.noPreview': 'Không xem trước được loại tệp này.',
  'viewer.versions': 'Phiên bản',
  'viewer.restore': 'Khôi phục bản này',
  'viewer.showingIn': 'Đang mở trong {where}.',
  'viewer.openedIn': 'Đã mở bằng {app}.',
  'viewer.defaultApp': 'ứng dụng mặc định',
  'viewer.nowCurrent': 'v{revision} giờ là phiên bản hiện tại.',
  'viewer.reading': 'Đang đọc…',
  'viewer.readingDoc': 'Đang đọc tài liệu…',
  'models.loadFailed': 'Không tải được danh sách model: {error}',
  'models.billedToKey': 'Model của {provider} — tính phí vào khoá {provider} của bạn.',
  'models.seesImages': 'đọc được ảnh',
  'models.noFreeHere': '{provider} không có model miễn phí — chúng tính phí vào khoá của bạn. Thử mục Miễn phí ở OpenRouter hoặc OrcaRouter.',
  'models.libraryUpdated': 'Đã cập nhật thư viện — {total} model.',
  'pages.artifacts.empty': 'Hãy yêu cầu một báo cáo, một bảng báo giá, một bảng tính hay một trang nhỏ và nó sẽ xuất hiện ở đây — và ở lại, bất kể nó ra đời từ cuộc trò chuyện nào.',
  'pages.idea.briefing.prompt': 'Tìm trên web xem 24 giờ qua có gì thay đổi trong các chủ đề tôi theo dõi, rồi viết cho tôi một bản tóm tắt ngắn. Nêu trước những gì thật sự đáng chú ý; nếu không có gì thì nói thẳng.',
  'pages.idea.watch.prompt': 'Tìm tin tức về [chủ đề] kể từ hôm qua. Nếu không có gì đáng kể, hãy nói gọn một dòng rồi dừng — đừng viết dài cho đủ.',
  'pages.idea.report.prompt': 'Tóm tắt những việc chúng ta đã làm trong tuần và tạo thành tệp .docx bằng create_file: đã làm gì, còn tồn đọng gì, và việc nào cần quyết định.',
  'pages.idea.tests.prompt': 'Trong thư mục làm việc của tôi, chạy bộ test và báo kết quả. Nếu có gì thất bại, hãy đưa phần output liên quan và nói bạn nghĩ sai ở đâu.',
  'time.minAgo': '{n} phút trước',
  'time.hoursAgo': '{n} giờ trước',
  'devices.moved': 'Đã lưu. Máy đó sẽ chuyển trong khoảng mười lăm giây.',
  'devices.switched': 'Đã đổi máy.',
  'devices.reallyUnpair': 'Huỷ ghép nối thật chứ?',
  'news.contextWindow': 'Cửa sổ ngữ cảnh',
  'news.yourKey': 'Khoá OpenRouter của bạn',
  'news.free': 'Cái này miễn phí — thử không mất gì.',
  'news.billed': 'Tính vào khoá OpenRouter của bạn theo mức giá ở trên.',
  'news.madeBy': 'Hãng làm',
  'news.released': 'Phát hành',
  'news.unstated': 'không nêu',
  'news.price': 'Giá',
  'news.priceFree': 'Miễn phí',
  'news.priceRate': '${in} vào · ${out} ra trên mỗi 1M token',
  'news.unpublished': 'chưa công bố',
  'news.runsOn': 'Chạy trên',
  'news.tokensM': '{n}M token',
  'news.tokensK': '{n}K token',
  'news.tokens': '{n} token',
  'news.nowDefault': '{model} giờ là model mặc định của bạn.',
  'compact.turnOn': 'Bật tự động nén',
  'compact.turnOff': 'Tắt tự động nén',
  'compact.onHint': 'Tự động gấp các lượt cũ lại trước khi đầy cửa sổ.',
  'compact.offHint': 'Cuộc trò chuyện sẽ ngừng hoạt động khi cửa sổ đầy.',
  'compact.isOn': 'Tự động nén đang bật.',
  'compact.isOff': 'Tự động nén đang tắt. Trò chuyện dài sẽ chạm giới hạn cửa sổ.',
  'compact.now': 'Nén ngay',
  'compact.nowHint': 'Tóm tắt các lượt trước và tiếp tục với phần chỗ trống vừa giải phóng.',
  'compact.nothing': 'Chưa có gì để nén.',
  'compact.working': 'Đang gấp các lượt trước lại…',
  'action.expandMenu': 'Mở rộng menu',
  'file.docx': 'Tài liệu Word',
  'file.xlsx': 'Bảng tính Excel',
  'file.pptx': 'Bản trình chiếu PowerPoint',
  'file.csv': 'Dữ liệu bảng tính',
  'file.html': 'Trang web',
  'file.txt': 'Văn bản thuần',
  'session.expired': 'Phiên đăng nhập đã hết hạn.',
  'account.yourPassword': 'Mật khẩu của bạn',
  'account.enterCode': 'Nhập mã 6 số',
  'devices.revertedToOwn': 'Đã lưu. Máy sẽ quay lại thiết lập riêng của nó.',
  'pages.tasks.lede': 'Công việc chạy theo lịch, hoặc chạy khi bạn bấm. Mỗi lần chạy nằm trong một cuộc trò chuyện riêng, đọc lại lúc nào cũng được.',

  /* ── phần khung trang: mọi nhãn và gợi ý ── */
  'taskForm.weekdayHint': 'Thêm thứ trong tuần ở đầu để chạy hằng tuần — <code>fri 16:00</code>. Giờ được tính theo múi giờ của trình duyệt này.',
  'behaviour.elsewhere': '<strong>Chế độ và mức suy luận</strong> nằm ở nút cạnh <em>Gửi</em>, và áp dụng ngay khi bạn bấm.',
  'skills.lede': 'Một quy trình dạy một lần thay vì giải thích lại ở mỗi cuộc trò chuyện. Trợ lý thấy tên và mô tả, và đọc toàn bộ hướng dẫn khi có kỹ năng phù hợp — nên chính <strong>mô tả</strong> quyết định kỹ năng có được dùng hay không. Trợ lý cũng có thể tự viết kỹ năng khi bạn dạy nó điều gì đó.',
  'admin.lede': 'Ai cũng có thể đăng ký bằng email và mật khẩu. Mỗi tài khoản dùng API key riêng và kết nối máy tính riêng — không bao giờ dùng của bạn. Đặt <code>ALLOW_SIGNUP=false</code> để đóng đăng ký khi mọi người bạn cần đã vào.',
  'tasks.repeat': 'lặp lại',
  'gate.ph.name': 'Tên của bạn',
  'gate.ph.password': 'Mật khẩu',
  'gate.ph.totp': 'Mã từ ứng dụng xác thực',
  'gate.ph.resetcode': 'Mã 6 chữ số',
  'account.newPassword': 'Mật khẩu mới',
  'gate.remember': 'Ghi nhớ tôi',
  'gate.forgot': 'Quên mật khẩu?',
  'nav.collapse': 'Thu gọn menu',
  'nav.closeMenu': 'Đóng menu',
  'nav.main': 'Điều hướng chính',
  'nav.scheduledTasks': 'Tác vụ chạy theo giờ',
  'nav.workspaceFiles': 'Tệp trong thư mục làm việc',
  'nav.conversationsLabel': 'Các cuộc hội thoại',
  'nav.openMenu': 'Mở menu',
  'pages.search': 'Tìm kiếm',
  'pages.searchPlaceholder': 'Tìm dự án…',
  'pages.clearSearch': 'Xoá tìm kiếm',
  'nav.breadcrumb': 'Đường dẫn',
  'proj.options': 'Tuỳ chọn dự án',
  'project.askPlaceholder': 'Hôm nay tôi có thể giúp gì cho bạn?',
  'project.start': 'Bắt đầu hội thoại',
  'approval.deny': 'Từ chối',
  'approval.allow': 'Cho phép và chạy',
  'vision.pickOne': 'Chọn model đọc được ảnh',
  'detail.progress': 'Tiến trình',
  'detail.close': 'Đóng bảng',
  'detail.empty': 'Khi trợ lý lên kế hoạch cho một việc nhiều bước, các bước sẽ hiện ở đây và được đánh dấu khi làm xong.',
  'screen.forward': 'Tiến tới',
  'screen.reload': 'Tải lại',
  'screen.alt': 'Màn hình trợ lý đang nhìn',
  'viewer.moreActions': 'Thao tác khác',
  'pair.offer': 'Máy tính này đang chờ được thêm. Nhập mã này từ bất kỳ thiết bị nào:',
  'pair.copyCode': 'Sao chép mã',
  'pair.oneLine': 'Một dòng lệnh để dán vào máy bạn muốn dùng. Nó tải Synapse về, kết nối với tài khoản này, và tự khởi động khi bạn đăng nhập.',
  'pair.setUp': 'Cài đặt một máy tính',
  'pair.orType': 'Hoặc nhập mã ghép nối',
  'pair.codeHint': 'Nếu máy đó đã chạy Synapse, nó sẽ hiện một mã 8 ký tự. Không nhất thiết phải là tài khoản của bạn, và làm được cả trên điện thoại.',
  'pair.submit': 'Ghép nối',
  'pair.yours': 'Máy tính của bạn',
  'news.badge': 'Model mới',
  'news.decline': 'Để sau',
  'news.apply': 'Dùng làm mặc định',
  'news.small': 'Dù chọn gì, bạn sẽ không bị hỏi lại về model này. Bạn luôn có thể đổi model cho từng cuộc trò chuyện bằng nút ở thanh trên cùng.',
  'models.title': 'Chọn model',
  'models.searchPlaceholder': 'Tìm — thử "free claude", ">200k", "<$1", hoặc dán id của model',
  'action.clear': 'Xoá',
  'models.filterProvider': 'Lọc theo nhà cung cấp',
  'models.all': 'Tất cả',
  'models.filterPrice': 'Lọc theo giá',
  'models.free': 'Miễn phí',
  'models.paid': 'Trả phí',
  'models.sort.new': 'Mới nhất trước',
  'models.sort.old': 'Cũ nhất trước',
  'models.sort.name': 'Tên A–Z',
  'models.sort.cheap': 'Rẻ nhất trước',
  'models.sort.context': 'Ngữ cảnh lớn nhất',
  'ws.findPlaceholder': 'Tìm trong các tệp này…  ↵',
  'ws.newFile': 'Tệp mới',
  'projForm.title': 'Tạo dự án',
  'projForm.nameLabel': 'Bạn đang làm gì?',
  'projForm.namePlaceholder': 'Đặt tên cho dự án',
  'projForm.aboutLabel': 'Bạn muốn đạt được điều gì?',
  'projForm.aboutPlaceholder': 'Mô tả dự án, mục tiêu, chủ đề…',
  'projForm.aboutHint': 'Phần này trở thành hướng dẫn thường trực của dự án — được mang vào mọi cuộc trò chuyện trong đó, nên không phải giải thích lại.',
  'projForm.create': 'Tạo dự án',
  'form.name': 'Tên',
  'textSource.namePlaceholder': 'Bản brief, ghi chú cuộc họp, đặc tả…',
  'textSource.content': 'Nội dung',
  'textSource.bodyPlaceholder': 'Dán vào đây.',
  'textSource.add': 'Thêm vào dự án',
  'projEdit.aboutHint': 'Hướng dẫn thường trực của dự án — được mang vào mọi cuộc trò chuyện trong đó.',
  'action.saveChanges': 'Lưu thay đổi',
  'wfForm.namePlaceholder': 'Báo cáo bán hàng thứ Hai',
  'wfForm.stepsPlaceholder': 'Lấy số liệu tuần trước\nVẽ biểu đồ\nGửi biểu đồ qua email cho cả nhóm',
  'wfForm.stepsHint': 'Mỗi dòng chạy lần lượt trong cùng một cuộc trò chuyện, nên một bước có thể dùng kết quả của bước trước. Một lượt chạy bị gián đoạn sẽ tiếp tục từ chỗ dừng — và bước đang chạy dở không bao giờ bị chạy lại, vì không có cách nào biết nó đã xong hay chưa.',
  'form.when': 'Khi nào',
  'wfForm.whenPlaceholder': 'mon 09:00, hoặc để trống',
  'form.frequency': 'Tần suất',
  'wfForm.repeat': 'Mỗi ngày hoặc mỗi tuần vào giờ đó',
  'form.once': 'Một lần rồi dừng',
  'taskForm.title': 'Tạo tác vụ chạy theo giờ',
  'taskForm.namePlaceholder': 'ban-tin-buoi-sang',
  'taskForm.promptPlaceholder': 'Tìm trên web những gì thay đổi qua đêm về các chủ đề tôi theo dõi và viết cho tôi một bản tin ngắn.',
  'taskForm.promptHint': 'Viết như khi bạn nói trong khung chat, vì đó chính xác là điều sẽ xảy ra: mỗi lần chạy là một lượt của một cuộc trò chuyện mới, với cùng công cụ và cùng quyền hạn.',
  'taskForm.whenPlaceholder': '08:00, hoặc fri 16:00',
  'taskForm.repeat': 'Mỗi ngày vào giờ đó',
  'providers.lede': 'Key được lưu trên máy chủ của bạn và không bao giờ gửi về trình duyệt. Nếu đặt chúng bằng biến môi trường thì ở đây chỉ xem được, không sửa được.',
  'behaviour.maxSteps': 'Số bước công cụ tối đa mỗi lượt',
  'behaviour.compact': 'Cuộc trò chuyện dài',
  'behaviour.compact.on': 'Gộp các lượt cũ thành bản tóm tắt trước khi đầy cửa sổ ngữ cảnh',
  'behaviour.compact.off': 'Giữ nguyên từng chữ — và dừng khi cửa sổ ngữ cảnh đầy',
  'behaviour.compact.hint': 'Mỗi lượt gửi lại toàn bộ cuộc trò chuyện, nên cuộc dài đến lúc sẽ không vừa nữa. Vòng tròn ở thanh trên cho biết đã đầy bao nhiêu; bạn cũng có thể tự gộp lại từ đó.',
  'behaviour.preview': 'Tài liệu mới',
  'behaviour.preview.on': 'Mở cạnh cuộc trò chuyện ngay khi được tạo',
  'behaviour.preview.off': 'Để dạng thẻ cho đến khi tôi bấm vào',
  'behaviour.preview.hint': 'Yêu cầu một bản báo cáo là để đọc nó. Tài liệu mở một lần mỗi lượt và đã đóng là đóng, nên không bật lại khi bạn đang làm việc — và không bao giờ mở trên màn hình hẹp, nơi bảng sẽ che mất cuộc trò chuyện.',
  'behaviour.theme': 'Giao diện',
  'behaviour.theme.system': 'Theo hệ thống',
  'behaviour.theme.dark': 'Tối',
  'behaviour.theme.light': 'Sáng',
  'behaviour.theme.hint': 'Áp dụng ngay, và chỉ cho trình duyệt này.',
  'behaviour.instructions': 'Hướng dẫn tuỳ chỉnh',
  'behaviour.instructionsPlaceholder': 'Trợ lý nên làm việc với bạn như thế nào?',
  'skills.namePlaceholder': 'Báo giá cước vận chuyển hằng tuần',
  'skills.when': 'Khi nào áp dụng',
  'skills.whenPlaceholder': 'Khi được yêu cầu báo giá một lô hàng container',
  'skills.procedure': 'Quy trình',
  'skills.procedurePlaceholder': '1. Kiểm tra bảng giá trong …',
  'skills.save': 'Lưu kỹ năng',
  'skills.saved': 'Đã lưu',
  'tasks.lede': 'Công việc chạy khi bạn không theo dõi. Mỗi lần chạy là một cuộc trò chuyện mới mà bạn mở xem sau — kết quả nằm chờ chứ không thông báo. Hãy viết yêu cầu như viết cho người không nhớ gì về hôm nay, vì đúng là như vậy.',
  'form.title': 'Tiêu đề',
  'tasks.titlePlaceholder': 'Tổng kết chiến dịch thứ Sáu',
  'tasks.what': 'Việc cần làm',
  'tasks.promptPlaceholder': 'Đọc các báo cáo trong D:\\campaigns và tóm tắt tuần này.',
  'tasks.whenPlaceholder': '17:00 hoặc fri 17:00',
  'tasks.schedule': 'Lên lịch',
  'connectors.lede': 'Token được mã hoá trên máy chủ và không bao giờ gửi lại trình duyệt. Mỗi token được kiểm tra với dịch vụ trước khi lưu, nên dán sai sẽ báo lỗi ngay ở đây chứ không phải giữa chừng công việc. Bạn có thể thu hồi trong cài đặt của chính dịch vụ bất cứ lúc nào.',
  'connectors.noGoogle': 'Gmail, Drive và Calendar không có ở đây: Google yêu cầu quy trình OAuth đầy đủ với một ứng dụng đã đăng ký, việc mà một ô nhập token không làm được. Thà không có còn hơn chạy nửa vời.',
  'worker.lede': 'Chạy Synapse trên máy bạn muốn trợ lý làm việc. Nó hiện một mã 8 ký tự — nhập mã đó và máy tính ấy thuộc về bạn trên mọi thiết bị bạn đăng nhập, lâu dài. Không có kết nối nào từ internet đi vào máy: chính máy tính chủ động kết nối ra ngoài.',
  'worker.byHand': 'Hoặc làm thủ công',
  'worker.enterCode': 'Nhập mã ghép nối',
  'account.displayName': 'Tên hiển thị',
  'account.changePassword': 'Đổi mật khẩu',
  'account.currentPassword': 'Mật khẩu hiện tại',
  'account.passwordPlaceholder': 'Ít nhất 10 ký tự',
  'account.updatePassword': 'Cập nhật mật khẩu',
  'account.passwordHint': 'Đổi mật khẩu sẽ đăng xuất mọi thiết bị khác — thiết bị này vẫn giữ đăng nhập.',
  'account.twofa': 'Xác thực hai lớp',
  'account.usage': 'Mức dùng tháng này',
  'account.signOut': 'Đăng xuất',
  'wfForm.steps': 'Các bước, mỗi dòng một bước',
  'taskForm.prompt': 'Tác vụ này cần làm gì?',
  'tasks.listHeading': 'Đã lên lịch',
  'projEdit.title': 'Sửa thông tin dự án',

  /* ── chuỗi do các module dựng: menu, thẻ, nhãn, trạng thái ── */
  'chat.untitled': 'Chưa đặt tên',
  'chat.pinned': 'Đã ghim',
  'chat.more': 'Thêm',
  'chat.pin': 'Ghim',
  'chat.unpin': 'Bỏ ghim',
  'chat.rename': 'Đổi tên',
  'chat.delete': 'Xoá',
  'chat.filesOne': '1 tệp',
  'chat.files': '{n} tệp',
  'proj.created': 'Đã tạo "{name}".',
  'worker.connected': 'Đã kết nối',
  'worker.notConnected': 'Chưa kết nối',
  'worker.workspaceLabel': 'Thư mục làm việc:',
  'worker.notTheOwner': 'Máy chủ này đang chạy trên máy tính của quản trị viên, và tệp cũng như shell của nó chỉ thuộc về tài khoản đó — ranh giới ấy là có chủ đích. Hãy nhờ quản trị viên nâng quyền tài khoản của bạn, hoặc ghép nối một máy của riêng bạn ở bên dưới; trợ lý sẽ làm việc trên máy đó.',
  'policy.menuTitle': 'Chế độ',
  'effort.named': 'Mức suy luận ({level})',
  'effort.set': 'Mức suy luận: {level}.',
  'keys.count': '{n} key',
  'keys.sharedFallback': 'Đang dùng tạm {envVar} của bản triển khai này, nên chi phí tính cho người đã cài đặt nó — và giới hạn token hằng tháng của bạn được áp dụng. Lưu key của riêng bạn bên dưới để bỏ cả hai.',
  'keys.add': 'Thêm',
  'keys.getOne': 'Lấy key →',
  'role.admin': 'quản trị viên',
  'role.member': 'thành viên',
  'skills.used': 'đã dùng {n} lần',
  'skills.savedNamed': 'Đã lưu "{name}".',
  'connectors.replace': 'Thay',
  'connectors.disconnect': 'Ngắt kết nối',
  'connectors.reallyDisconnect': 'Ngắt kết nối nhé?',
  'usage.calls': '{n} lượt gọi',
  'usage.sharedLimit': 'Đã dùng {pct}% trong {limit} token key dùng chung của bạn tháng này. Thêm API key của riêng bạn để bỏ giới hạn.',
  'admin.you': 'bạn',
  'admin.limitButton': 'Giới hạn',
  'admin.suspend': 'Tạm khoá',
  'admin.unsuspend': 'Mở khoá',
  'admin.limitPlaceholder': 'token / tháng, 0 = không giới hạn',
  'status.saved': 'Đã lưu.',
  'context.gauge': '{percent}% cửa sổ ngữ cảnh — khoảng {used}K trên {total}K token',
  'context.gaugeEstimated': '{percent}% cửa sổ ngữ cảnh — khoảng {used}K trên {total}K token (ước tính)',
  'context.menuUsed': 'Đã dùng {percent}% · {used} trên {total} token',
  'search.searching': 'Đang tìm…',
  'models.contextM': 'ngữ cảnh {n}M',
  'models.contextK': 'ngữ cảnh {n}K',
  'models.context': 'ngữ cảnh {n}',
  'models.pricePer1M': '${in} / ${out} mỗi 1M',
  'models.today': 'hôm nay',
  'models.monthsAgo': '{n} tháng trước',
  'models.yearsAgo': '{n} năm trước',
  'common.loading': 'Đang tải…',
  'models.libraryStatus': '{total} model · {free} miễn phí · cập nhật {when}',
  'models.tagFree': 'miễn phí',
  'models.tagNew': 'mới',
  'models.tagBuiltIn': 'có sẵn',
  'models.noCost': 'không mất phí',
  'models.released': 'phát hành {when}',
  'pages.edited': 'Sửa {when}',
  'pages.searchNamed': 'Tìm trong {name}…',
  'chat.openThisFile': 'Mở tệp này',
  'screen.tabTitle': 'Tab {n}: {host}',
  'screen.sourceDesktop': 'máy tính',
  'screen.sourceSandbox': 'hộp cát',
  'viewer.notes': 'Ghi chú',
  'viewer.tab.text': 'Văn bản',
  'proj.noChats': 'Dự án này chưa có cuộc trò chuyện nào.',
  'proj.noChatsHint': 'Mọi điều bạn hỏi ở trên sẽ bắt đầu và nằm lại ở đây, với hướng dẫn và nguồn của dự án đã sẵn sàng.',
  'proj.instructions': 'Hướng dẫn',
  'proj.memory': 'Trí nhớ',
  'proj.memoryLede': 'Những gì trợ lý đã ghi nhớ về bạn. Đây là <strong>cùng một bộ ghi chú ở mọi dự án</strong> và trong các cuộc trò chuyện thường — không phải trí nhớ riêng của dự án này.',
  'proj.memoryEmpty': 'Chưa ghi nhớ gì. Hãy nói với trợ lý điều đáng lưu lại — bạn thích văn phong thế nào, bạn đang làm gì — và nó sẽ lưu một ghi chú. Các ghi chú đó dùng chung cho mọi dự án trong tài khoản này, không riêng dự án nào.',
  'proj.context': 'Ngữ cảnh',
  'proj.dropHere': 'Thả tệp vào đây, hoặc bấm để chọn',
  'proj.dropKinds': 'PDF, Word, Excel, PowerPoint, văn bản và mã nguồn',
  'proj.sourcesAsText': 'Nguồn được lưu dưới dạng văn bản đọc ra từ tệp, và đó là thứ trợ lý trích dẫn.',
  'proj.reading': 'Đang đọc {name}… ({n}/{total})',
  'wf.interrupted': 'Một bước đã bị gián đoạn và <strong>không tự động chạy lại</strong> — không có cách nào biết việc của nó đã xảy ra hay chưa. Hãy xem lại cuộc trò chuyện, rồi chạy lại nếu vẫn cần.',
  'ws.unsaved': 'chưa lưu',
  'ws.reading': 'Đang đọc…',
  'ws.created': 'Đã tạo {path}.',
  'ws.root': 'thư mục làm việc',
};
